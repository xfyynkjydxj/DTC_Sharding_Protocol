...
class ViewState
{
    ...
    size_t accepted_confirm = 0;//接收确认时间
    size_t accepted_pre_prepare = 0;//预准备时间
...
//    ViewState(Message & msg);
    ...
    ViewState & operator = (const ViewState& vt); //重载操作符 =
    explicit ViewState(const Message& msg);
    void handle_message(Message msg, Node & node); //处理消息 
